...
class EBest:
    ...
    def _execute_query(self, res, in_block_name, out_block_name, *out_fields, **set_fields):
        ...
        :param out_fields:출력 필드 리스트
        :param set_fields:인 블록에 설정할 필드 딕셔너리
        ...
        while len(self.query_cnt) >= EBest.QUERY_LIMIT_10MIN:
            ...
            self.query_cnt = list(lambda x: (datetime.today() - x).total_seconds() < EBest.LIMIT_SECONDS,
                                  self.query_cnt)
    ...
    def get_stock_price_by_code(self, code=None, cnt="1"):
        ...
        out_params = ['date', 'open', 'high', 'low', 'close', 'sign',
                      ...
                      'l_diff', 'marketcap']
        result = self._execute_query("t1305", "t1305InBlock", "t1305OutBlock1",
    ...
    def get_credit_trend_by_code(self, code=None, date=None):
        """TR: t1921 신용거래동향
        :param code:str 종목코드
        :param date:str 날짜 8자리 ex) 20210203
        """
